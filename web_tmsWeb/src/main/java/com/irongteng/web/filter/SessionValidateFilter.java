package com.irongteng.web.filter;
...
public class SessionValidateFilter implements Filter {
    ...
    private static final Log logger = LogFactory.getLog(SessionValidateFilter.class);
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (!validator.validate()) {
            ...
            if (ServerInfo.isAjax(request) || request.getParameter("ajax") != null) {
                ...
                out.println("{\"statusCode\":\"301\", \"message\":\"用户登录超时，请重新登录! !\"}");
            } else {
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        if (config.getInitParameter(LOGIN_URL) != null) {
            StringBuilder sb = new StringBuilder(config.getInitParameter(LOGIN_URL));
            if (sb.indexOf("?") >= 0) {
                sb.append("&");
            } else {
                sb.append("?");
