 ...
 */
public class SiemensModem extends ModemBase {

    /**
     * AT指令：选择短信格式——PDU模式或者Text模式
     */
    private final static String AT_CMGF = "+CMGF";

    /**
     * AT指令：新短信提示方式
     */
    private final static String AT_CNMI = "+CNMI";

    /**
     * AT指令：设置文本格式参数
     */
    private final static String AT_CSMP = "+CSMP";

    /**
     * AT指令：短信中心电话号码
     */
    private final static String AT_CSCA = "+CSCA";

    /**
     * AT指令：读短信
     */
    private final static String AT_CMGR = "+CMGR";

    /**
     * AT指令：发送短信
     */
    private final static String AT_CMGS = "+CMGS";

    /**
     * AT指令：删除短信
     */
    private final static String AT_CMGD = "+CMGD";

    /**
     * AT指令：新短信
     */
    private final static String AT_CMTI = "+CMTI";

    /**
     * AT指令：SIM卡是否存在
     */
    private final static String AT_CIMI = "+CIMI";
     ...
     *            modem配置信息
     */
    public SiemensModem(Vector<ReadRptDataListener> listener, ModemInfo modemInfo) {
     ...
     *            基偶校验
     */
    public SiemensModem(Vector<ReadRptDataListener> listener, ModemInfo modemInfo, int flowControlIn, int flowControlOut, int databits,
    ...
    protected void informReadData(String data) {
        ...
        /**
         * // 如果包含 "+CMTI"，调用analyzeNewShortMsg(s)通知收到短信 else if (data.length()
         * > 5 && data.substring(0, 5).equals(AT_CMTI)) {
         * analyzeNewShortMsg(data); } // 如果包含“+CMGR”，调用analyzeCMGR(s)通知收到读短信应答
         * else if (data.length() > 5 && data.substring(0, 5).equals(AT_CMGR)) {
         * if (data.indexOf("REC UNREAD") > 0 && data.indexOf("\r\n") < 0) {
         * isReceiveFull = true; tempStr = tempStr + data; } else {
         * analyzeCMGR(data); } }
         */
        // 如果包含“+CMGS”，调用analyzeCMGS(s)通知收到发送短信应答
        else if (data.length() > 5 && data.substring(0, 5).equals(AT_CMGS)) {
            analyzeCMGS(data);
        ...
        if (data.length() > 5 && data.contains(AT_CMGR)) {
            if (data.indexOf("REC UNREAD") > 0 && !data.contains("\r\n")) {
                ...
                System.out.println("收到的短信内容为：" + data);
        ...
        for (String sData : splitData) {
            ...
            if (i >= 0) {
                try {
                    ...
                    // Timer readMsgTimer = new Timer("readMsgTimer");
                    // CommLog.writeTestLog("创建处理新短信定时器：readMsgTimer");
                    // readMsgTimer.schedule(new TimerTask() {
                    // public void run() {
                    //     readAndDeleteSms(newSmsIndex);
                    // //readShortMsg(newSmsIndex);
                    // }
                    // }, 1);
    ...
    synchronized public boolean sendEnglishSMS(String destPhone, String content) {
        ...
        try {
            ...
            /**
             * try { Thread.sleep(100); Thread.yield(); } catch(Exception e){}
             */
            return !(atCmd == null || !atCmd.getResponse() || !atCmd.getStatus());
        } finally {
