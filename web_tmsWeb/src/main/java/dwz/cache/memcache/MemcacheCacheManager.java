 ...
 * 1. 读取配置文件 /memcached.xml 2. 初始化SockIOPool. 3. 初始化MemcacheClient.
 * <br>
...
public class MemcacheCacheManager implements CacheManager {
    ...
    private final String MEMCACHED_CONFIG = "/memcached.xml";
    ...
    protected void init() {
        try {
            ...
            if (logger.isDebugEnabled()) {
                ...
                poolSB.append("failover:").append(this.memcachedClientSocketPoolConfig.isFailover()).append("\n");
                poolSB.append("initConn:").append(this.memcachedClientSocketPoolConfig.getInitConn()).append("\n");
                poolSB.append("minConn:").append(this.memcachedClientSocketPoolConfig.getMinConn()).append("\n");
                poolSB.append("maxConn:").append(this.memcachedClientSocketPoolConfig.getMaxConn()).append("\n");
                poolSB.append("maintSleep:").append(this.memcachedClientSocketPoolConfig.getMaintSleep()).append("\n");
                poolSB.append("nagle:").append(this.memcachedClientSocketPoolConfig.isNagle());
                poolSB.append("socketTo:").append(this.memcachedClientSocketPoolConfig.getSocketTo());
                poolSB.append("aliveCheck:").append(this.memcachedClientSocketPoolConfig.isAliveCheck());
                poolSB.append("maxIdle:").append(this.memcachedClientSocketPoolConfig.getMaxIdle());
                for (int i = 0; i < this.memcachedClientSocketPoolConfig.getServers().length; i++) {
                    poolSB.append("server:").append(memcachedClientSocketPoolConfig.getServers()[i]).append("weight : ").append(memcachedClientSocketPoolConfig.getWeights()[i]);
                }

                logger.debug(poolSB.toString());

                StringBuilder clientSB = new StringBuilder();
                clientSB.append("compressEnable:").append(memcachedClientConfig.isCompressEnable()).append("\n");
                clientSB.append("defaultEncoding:").append(memcachedClientConfig.getDefaultEncoding()).append("\n");
                clientSB.append("errorHandler:").append(memcachedClientConfig.getErrorHandler()).append("\n");
                clientSB.append("primitiveAsString:").append(memcachedClientConfig.isPrimitiveAsString()).append("\n");
                clientSB.append("compressThreshold:").append(memcachedClientConfig.getCompressThreshold()).append("\n");
